...
def load_vgg(sess, vgg_path):
    ...
    # STep1: Use tf.saved_model.loader.load to load the model and weights
    vgg_tag = 'vgg16'
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # Step 2: Implement layers function
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    """
    # Step 3: make logits a 2D tensor where each row represents a pixel and each column a class
    # flatten images
    ...
    # manually add regularization to the cross-entrpy-loss
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # Step 5: Build NN using load_vgg, layers, and optimize function
        input_image, keep_prob, vgg_layer3_out, vgg_layer4_out, vgg_layer7_out = load_vgg(sess, vgg_path)
        nn_last_layer = layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes)

        # placeholders for label and learning-rate
        ...
        # Step 6: Train NN using the train_nn function
        sess.run(tf.global_variables_initializer())
        train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image, correct_label, keep_prob, learning_rate)

        # Step 7: Save inference data using helper.save_inference_samples
